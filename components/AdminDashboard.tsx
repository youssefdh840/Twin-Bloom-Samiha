...
const AdminDashboard: React.FC<{ onExit: () => void }> = ({ onExit }) => {
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!imageUrl) {
        throw new Error("Image upload failed. Please check your Supabase Storage bucket settings and permissions.");
      }
      ...
      if (result) {
        ...
        alert("The image uploaded, but we couldn't save the product details to the database. Ensure your 'products' table has columns: 'name', 'price', 'image_url', and 'paired_scent_id'.");
      }
    } catch (err: any) {
      console.error(err);
      alert(err.message || "An unexpected error occurred during publishing.");
    } finally {
                ...
                {!supabase ? (
                  ...
                  outfits.map((outfit) => (
                    <tr key={outfit.id} className="group hover:bg-gray-50 transition-colors">
                      <td className="py-4">
                        <img src={outfit.imageUrl} className="w-12 h-12 object-cover rounded-sm border border-gray-200" alt="" />
                      </td>
                      <td className="py-4">
                        <div className="text-sm font-medium text-black">{outfit.name}</div>
                        <div className="text-[10px] text-gray-400 uppercase tracking-tighter">
                          Pair: {FRAGRANCES.find(f => f.id === outfit.pairedScentId)?.name || 'N/A'}
                        </div>
                      </td>
