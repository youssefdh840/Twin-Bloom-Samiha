...
const AdminDashboard: React.FC<{ onExit: () => void }> = ({ onExit }) => {
  ...
  const [newOutfit, setNewOutfit] = useState({ name: '', price: '' });
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await storeService.addOutfit({
        ...
        pairedScentId: '', // Default to empty as fragrances are removed
        imageUrl
      });
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-50">
                {outfits.length === 0 ? (
                  <tr><td colSpan={3} className="py-12 text-center text-xs text-gray-400 italic">Inventory is empty.</td></tr>
                ) : (
                  outfits.map((outfit) => (
