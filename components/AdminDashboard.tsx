...
const AdminDashboard: React.FC<{ onExit: () => void }> = ({ onExit }) => {
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    const client = getSupabase();
    if (!client) {
      alert("Supabase is not initialized. Please ensure VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY are correctly set in your environment.");
      return;
    }
    if (!selectedFile) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {!hasClient && (
          <div className="mb-8 p-4 bg-red-50 border border-red-100 rounded-sm">
            <p className="text-xs text-red-600 font-medium uppercase tracking-widest leading-relaxed">
              Error: Supabase configuration or CDN script missing. <br/>
              Please verify VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY are added to your Vercel Environment Variables and that the CDN script has loaded.
            </p>
          </div>
        )}
        ...
        <section className="bg-white rounded-sm shadow-sm p-8 mb-12 border border-gray-100">
          <div className="flex justify-between items-center mb-8">
            ...
            <button 
              ...
              className="bg-black text-white px-6 py-2 text-xs uppercase tracking-widest hover:bg-matte-gold transition-colors disabled:opacity-30"
            >
                ...
                {!hasClient ? (
                  <tr>
                    <td colSpan={4} className="py-12 text-center text-xs text-gray-400 italic">
                      Backend services unavailable due to configuration error.
                    </td>
                  </tr>
                ) : outfits.length === 0 ? (
                  <tr>
