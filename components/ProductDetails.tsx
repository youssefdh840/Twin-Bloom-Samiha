...
const ProductDetails: React.FC<ProductDetailsProps> = ({ outfit, onBack }) => {
  ...
  // Initialize with the first available in-stock size
  ...
  useEffect(() => {
    ...
    if (!catalogSizes.includes(selectedSize)) {
  ...
  const handleWhatsAppOrder = () => {
    ...
    const orderStatus = isOOS ? " (Note: This size is currently Out of Stock, please confirm availability)" : "";
    const message = `Hello, I would like to order the "${outfit.name}" in size "${selectedSize}". (Price: ${outfit.price})${orderStatus}. Please let me know how to proceed with the purchase.`;
